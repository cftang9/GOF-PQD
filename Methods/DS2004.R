...
DS_C <- function(Data,m=NULL){
  n = length(Data[,1])
  # the suggested number of marginal grids points
  if(is.null(m) == T){
    if(n<100){m=3}
    if(n>=100&n<200){m=4}
    if(n>=200&n<400){m=5}
  ...
  #estimating the difference of estimated copula and independence copula
  Cn = array(,c(m,m));
  Fn_1 = array(,m);
  Gn_1 = array(,m);
  for(i in 1:m){
  ...
  if(Singular.Condition==F){
    ...
    #estimating p-value and critical values by generating multivariate normal distributions
    BB = 1000; 
    wddi = array(0,(d+1));
    for(bb in 1:BB){
    ...
    #estimating critical values for significance level 0.05
    grids = seq(0,20,by=0.001); 
    probability = array(0,length(grids)); 
    for(k in 1:length(grids)){
      for(j in 1:d){
        probability[k] = probability[k] + pchisq(grids[k], df = j, lower.tail=FALSE)*wddi[j]; 
      }
    }
    CV = grids[which.min(abs(probability-0.05))]
    
    return(list(TS = as.numeric(eta_C), 
                CV = CV, 
                pvalue = pvalue, 
                Rejection=c(pvalue<0.05), 
                singular_V_F = Singular.Condition));
  }
