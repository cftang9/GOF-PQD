EL_C_Fin_n100 <- function(Data){
  X = Data[,1]; Y = Data[,2]; 
  n = length(X); 
  rX = rank(X)/n; rY = rank(Y)/n
  X = rX; Y=rY
  ...
  ELn = mean(LLn1)
  Reject = c(ELn>1.1227709); 
  return(list(TS=ELn, Rejection=Reject))
}
EL_Fin_n100 <- function(Data){
  X = Data[,1]; Y = Data[,2]; 
  n = length(X); 
  Hn = array(,n); 
  An11 = Hn; An12 = Hn; An21 = Hn; An22 = Hn;
...
Data = cbind(rpois(10,5),rpois(10,5))

T1 = EL_Fin_n100(Data)
T2 = EL_C_Fin_n100(Data)
