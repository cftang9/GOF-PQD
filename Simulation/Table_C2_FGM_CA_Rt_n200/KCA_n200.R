PQD.Dist.Joint.KS.n200 <- function(Data){
  X = Data[,1]; Y = Data[,2]; 
  n = length(X); 
  ...
  return(list(TS = KS, 
              Rejection = c(KS>0.6717514)
  )); 
  
}

PQD.Dist.Joint.AD.n200 <- function(Data){
  X = Data[,1]; Y = Data[,2]; 
  n = length(X); 
  ...
  AD = sum( (pmax(Fn*Gn-C_n,0))^2/(u_n*v_n*(1-u_n)*(1-v_n))); 
  
  return(list(TS = AD, 
              Rejection = c(AD>1.1768929))
  ); 
